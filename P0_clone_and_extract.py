...
for k, commit in enumerate(tqdm(commits, total=len(commits))):
    ...
    for diff in commit.diff(commits[k - 1], paths=local_name):
        ...
        # Fix weird bug with naming (changes at some point...)
        d0 = d0.rename(columns=lambda x: x.title())
        d1 = d1.rename(columns=lambda x: x.title())
        ...
        for idx in common_index:
            r0 = d0.loc[idx].to_dict()
            r1 = d1.loc[idx].to_dict()
